...
describe('Searchable encryptor service tests', () => {
    test('Check cache hit or miss', async () => {
        ...
        const result = SearchableEncryptorService._searchCacheByKeywords(supposedlyEncryptedKeywords);
        await expect(result).resolves.not.toThrow();
    });

    test('Search using encrypted keywords', async () => {
        let supposedlyEncryptedKeywords: Array<string> = ["power", "love", "Dostoyevski"];
